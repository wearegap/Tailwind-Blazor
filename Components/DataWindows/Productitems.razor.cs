// @pb-anchor source=d_productitems.srd kind=datawindow-codebehind
...
{
    [Inject] private ProductitemsService _svc { get; set; } = default!;
    [Inject] private ISnackbar Snackbar { get; set; } = default!;
    [Inject] private ImageCatalog ImageCatalog { get; set; } = default!;

    internal IReadOnlyList<ProductitemsDto> _rows = Array.Empty<ProductitemsDto>();
    internal ProductitemsDto? _row;
    internal bool _loading;

    internal int _currentIndex;

    internal string? ResolvedImageSrc_Imagename => ImageCatalog.Resolve(_row?.Imagename);
